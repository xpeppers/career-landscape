...
class UserViewTest(TestCase):
    ...
    def test_user_view_display_correct_dimension_value(self):
        ...
        scores = [ ScoreFactory.create(person=user, dimension=dimension, kind=i ) for i in range(0,3) ]

        response = client.get(f'/users/{user.id}', { 'selected_user_circle' : circle.id }, follow=True)

        self.assertContains(response, f'<span class="badge badge-secondary red-bg">{scores[0].value}')
        self.assertContains(response, f'<span class="badge badge-secondary yellow-bg">{scores[1].value}')
        self.assertContains(response, f'<span class="badge badge-secondary green-bg">{scores[2].value}')
    ...
    def test_user_view_momentum_filter_is_in_template_context_if_applied(self):
        ...
        response = client.get(f'/users/{user.id}', { 'selected_user_circle' : circle.id, 'momentum' : 'on', 'selected_topic' : topic.name }, follow=True)
    ...
    def test_user_view_next_step_filter_is_in_template_context_if_applied(self):
        ...
        response = client.get(f'/users/{user.id}', { 'selected_user_circle' : circle.id, 'next-step' : 'on', 'selected_topic' : topic.name }, follow=True)
